...
const Conversation: React.FC = () => {
  ...
  const { comments, conversation, activity, appConfig, peerUser } = useMemo(() => {
    ...
    const _activity: API.Activity | undefined = activities?.find(
      (item): boolean => item.activitiesId === _conversation?.activitiesId,
    );
  ...
  return (
    <PageContainer>
          ...
          <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
            {activity.activitiesImages?.map((imagePath) => {
              <Image
                ...
                height={200}
                src={`${
                  appConfig?.uCloudPublicStoreUrl ?? 'https://private-image.tideswing.fun'
                }${imagePath}`}
              />;
            })}
          </div>
      ...
      >
        {comments.length === 0 || comments[0].commentType !== TSCommentType.light ? (
          <p>查看更多</p>
        ) : null}
      </div>
      <List
      ...
      <Input.Group compact>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            const list = commentsMap.get(commentTag ?? '') ?? [];
            list.push({
              ...comment,
              commentUser: '1',
            });
            commentsMap.set(commentTag ?? '', list);
            setInitialState({
              ...initialState,
              commentsMap,
            });
